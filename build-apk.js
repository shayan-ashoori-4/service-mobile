...
function askForUrl() {
  return new Promise((resolve) => {
    ...
    rlInterface.question('🌐 Enter website URL: ', (answer) => {
      resolve(answer.trim());
    });
  });
}
...
function askForAppName() {
  return new Promise((resolve) => {
    const rlInterface = getReadline();
    rlInterface.question('📱 Enter application name: ', (answer) => {
      resolve(answer.trim());
    });
  });
}
...
function askForPackageName() {
  return new Promise((resolve) => {
    const rlInterface = getReadline();
    rlInterface.question('📦 Enter package name (e.g., com.example.app): ', (answer) => {
      resolve(answer.trim());
    });
  });
}
...
async function buildApk(providedUrl, providedAppName, providedPackageName) {
  let webUrl, appName, packageName;

  // Check if arguments are provided via command line or function parameters
  if (providedUrl && providedAppName && providedPackageName) {
    // Use provided arguments
    webUrl = providedUrl;
    appName = providedAppName;
    packageName = providedPackageName;
  } else if (process.argv.length >= 5) {
    // Use command line arguments: node build-apk.js <url> <appName> <packageName>
    webUrl = process.argv[2];
    appName = process.argv[3];
    packageName = process.argv[4];
  } else {
    // Interactive mode - ask for input
    webUrl = await askForUrl();
    if (!webUrl) {
    ...
    packageName = await askForPackageName();
    if (rl) rl.close();
  }
  ...
  try {
    ...
    // Clear Metro bundler cache before building (non-blocking)
    console.log('\n🧹 Clearing Metro bundler cache...');
    try {
      // Just clear the cache directory without starting Metro
      const cacheDir = path.join(liteServiceDir, 'node_modules', '.cache');
      if (fs.existsSync(cacheDir)) {
        fs.rmSync(cacheDir, { recursive: true, force: true });
        console.log('✅ Metro cache cleared');
      } else {
        console.log('✅ No Metro cache found to clear');
      }
    } catch (error) {
      // Ignore cache clear errors
      console.log('⚠️  Cache clear skipped:', error.message);
    }
